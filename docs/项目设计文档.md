...
| 显示模块 | `Renderer` | 打印棋盘、分数和提示 |
| 输入模块 | `InputHandler` | 读取和解析用户输入 |
...
class Renderer {
    +clearScreen() void
    +render(Board board, int score, int bestScore) void
    +showWelcome() void
    +showGameOver() void
    +showWin() void
...
`Renderer::render()` 使用固定宽度格式化输出棋盘。空格显示为 `.`，非零数字右对齐显示，保证棋盘整齐。

### 6.5 分数更新

每次合并得到的新数字会累加到 `gainedScore`。如果本次移动有效，`Game` 调用 `ScoreManager::addScore()` 更新当前分数和最高分。
